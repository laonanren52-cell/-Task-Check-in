...
export function AppShell() {
  ...
  const safeMode = isSafeMode(location.search)
  ...
  return <main className={`app-shell overflow-x-hidden w-full max-w-full ${safeMode ? 'safe-mode' : ''}`}>
    ...
    <div className="workspace">
      <motion.div key={location.pathname} initial={reduceMotion || safeMode ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion || safeMode ? 0 : 0.28, ease: [0.16, 1, 0.3, 1] }}>
    ...
    {!safeMode && <button className="floating-copilot" aria-label="打开 AI 学习助手" onClick={() => setCopilot(true)}><Sparkles /><span>AI</span></button>}

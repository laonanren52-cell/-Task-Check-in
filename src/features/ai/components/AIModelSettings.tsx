...
export function AIModelSettings() {
  ...
  return <section className="ai-settings">
    ...
    <section className="ai-permissions"><div><span className="eyebrow">输出语言</span><h3>让 AI 用你习惯的语言表达。</h3><p>只影响 AI 新生成的自然语言内容；任务名称、学习科目、项目名与技术术语会保留原文。</p></div><label className="ai-language-select"><span>AI 输出语言</span><select value={aiPreferences.outputLanguage} onChange={event => saveAIPreferences({ ...aiPreferences, outputLanguage:event.target.value as typeof aiPreferences.outputLanguage, updatedAt:new Date().toISOString() })}><option value="follow-app">跟随软件语言</option><option value="zh-CN">简体中文</option><option value="en-US">English</option></select><small>当前中文界面下，“跟随软件语言”会使用简体中文。</small></label></section>

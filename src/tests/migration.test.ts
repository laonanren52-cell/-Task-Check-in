...
import { convertLegacyBackup, convertLegacyData, migrateLegacyStorage, parseLegacyData } from '../features/migration/legacy'
...
describe('旧版 localStorage 迁移',()=>{
  ...
  it('可把旧版 JSON 备份转换为当前可导入格式',()=>{
    const backup=convertLegacyBackup(raw)
    expect(backup).toMatchObject({schemaVersion:2})
    expect(backup.tasks[0]).toMatchObject({plannedDuration:90,actualDuration:60,status:'done'})
    expect(backup.dailyRecords[0]).toMatchObject({date:'2026-07-16',checkedIn:true})
  })

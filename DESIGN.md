...
- 设置页使用 12 列无空洞网格：版本摘要 7 列、状态 5 列，更新策略与自动检查开关保持同一节奏。
- 更新弹窗沿用冰川蓝和细边框，不使用系统警告框、庆祝动画或虚假进度。
- 版本变化、更新说明、发布时间、下载大小和当前阶段分层呈现；主按钮仅用于下载或重启安装。
- 下载进度只使用 Tauri updater 回调的真实字节数据；总大小未知时明确写明，不伪造百分比。
- 开发模式、Web/PWA 和 Tauri 桌面环境使用可区分但低饱和的状态标识。
